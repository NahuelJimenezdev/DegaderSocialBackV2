...
async function verifyNotification() {
    try {
        ...
        const superior = await User.findOne({ email: 'departamental@gmail.com' });

        if (!applicant || !superior) {
            console.error('❌ No se encontraron los usuarios necesarios.');
            process.exit(1);
        }

        console.log(`🔍 Probando notificación: ${applicant.email} (Municipal) -> ${superior.email} (Departamental)`);

        // Simulamos la lógica de fundacionController.js (solicitarUnirse)
        ...
        console.log(`🔎 Area Core: ${areaCore}, Regex: ${areaRegex}`);
        ...
        // Verificar en DB
        const finalNotif = await Notification.findOne({ receptor: superior._id, emisor: applicant._id });
        if (finalNotif) {
            console.log('✅ VERIFICACIÓN EXITOSA: La notificación llegó al Director Departamental.');
        } else {
            console.log('❌ VERIFICACIÓN FALLIDA: La notificación no llegó al Director Departamental.');
        }
